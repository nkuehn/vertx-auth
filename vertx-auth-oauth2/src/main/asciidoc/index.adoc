...
The provider defaults to treating tokens as JWT tokens but can be configured to treat tokens as opaque strings only.
...
3. If a role is provided then the lookup happens in the `resource_access` list under the role name
...
error. This is because the OAuth2 might have received a request to invalidate the token or a logout in between.
